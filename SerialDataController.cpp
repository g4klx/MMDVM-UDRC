...
bool CSerialDataController::open(const std::string& device, SERIAL_SPEED speed)
{
	...
	std::string baseName = device.substr(4U);		// Convert "\\.\COM10" to "COM10"

	m_handle = ::CreateFile(device.c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
